...
import { MessageData, Attachment } from "@/Types/Chat";

const MessageBox: React.FC<MessageData> = ({ data }) => {
  ...
  const ImageModal: React.FC<{ src: string; alt: string }> = ({ src, alt }) => {
      ...
      <div className={body}>
        ...
        {(data?.attachments ?? []).length > 0 ? (
          (data.attachments ?? []).map((attachment) => (
            <div key={attachment._id}>{renderAttachment(attachment)}</div>
          ))
        ) : (
